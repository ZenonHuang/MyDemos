...
#warning 注意：以下方法循环模拟了通过 userIds 同步查询 users 信息的过程，这里需要替换为 App 的 API 同步查询
          ...
          [userIds enumerateObjectsUsingBlock:^(NSString *_Nonnull clientId, NSUInteger idx,
            ...
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
              ...
              api.baseUrl = [NSString stringWithFormat:@"http://quseit.cn:10011/api/user/info?social=facebook&id=729606240511552&user_id=%@", clientId];
              [api setApiCompletionHandler:^(id _Nonnull response, NSError *_Nullable error) {
                if (error) {
                    //注意：如果网络请求失败，请至少提供 ClientId！
                    LCCKUser *user_ = [LCCKUser userWithUserId:clientId name:@"UnKnown" avatarURL:nil clientId:clientId];
                    ...
                    LCCKUser *user_ = [LCCKUser userWithUserId:clientId name:@"known" avatarURL:nil clientId:clientId];
                    [users addObject:user_];
                    // 发送信号量
                    dispatch_group_leave(group);
                }
